"""
Threads search — multi-strategy approach.
...
def _format_result(url: str, title: str = "", platform_id: str = "") -> Dict:
    ...
    return {
        ...
        "title": title[:200] if title else f"[Threads] {url}",
        "description": "",
        "url": url,
        "thumbnail_url": "",
        "author_name": "",
...
def _generate_search_urls(query: str) -> List[Dict]:
    ...
    return [
        _format_result(
            url=f"https://www.threads.net/search?q={encoded}",
            title=f"{query} — Threads Search",
